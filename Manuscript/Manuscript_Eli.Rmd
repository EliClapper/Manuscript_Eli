...
abstract: |
  Here I will write an abstract about my manuscript. currently I do not have one so
  I use sample text to fill up the missing abstract
...
<br>
  ...
  * *Rouge* (Recall-oriented understudy for gisting evaluation): Rouge is an algorithm developed by @lin2003automatic to automatically evaluate summaries based on n-gram co-occurrence with a model summary. A model summary in this thesis is a randomly sampled summary that got the highest rating. 

*MaxCopy, CopiedSumm, CopiedPass, FirstSent* and *Rouge* reflect gist extraction. *SentLength* and *WordLength* may explain rater variability not explained by the rubric. 
